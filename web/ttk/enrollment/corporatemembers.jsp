...
long policy_num,productTyp_num,insComp_num,grpId_num,EnrollBatchSeqID;
...
<%  policy_status =(String)request.getSession().getAttribute("policy_status");
	...
	if(request.getSession().getAttribute("EnrollBatchSeqID")!= null){
	EnrollBatchSeqID = ((Long)request.getSession().getAttribute("EnrollBatchSeqID")).longValue();
	}
	...
	<html:errors/>
     <table align="center" class="searchContainer rcContainer" border="0" cellspacing="0" cellpadding="0">
      <tr>
       ...
       <logic:equal name="strSublink" value="Corporate Policy" >
        ...
        <td nowrap>Order No.:
         ...
         <td nowrap>Member ID:<br>
		    	<html:text property="sMemberID" name="frmMember"  styleClass="textBox textBoxMedium" maxlength="250"/>
		 </td>
       ...
       <%--  <a href="#" onClick="javascript:window.open('<%=softCopyUrl%>', '', '', '')" >Soft Copy Enrolment Upload</a></td> --%>
       <a href="#" onClick="javascript:softcopyUpload()" >Soft Copy Enrolment Upload</a></td>
	  ...
	  <tr>
		<td align="right" colspan="2" class="buttonsContainerGrid">
			...
			<%
				...
				{
		     %>
				     <button type="button" name="Button" accesskey="a" class="buttons" onMouseout="this.className='buttons'" onMouseover="this.className='buttons buttonsHover'" onClick="javascript:addNewEnrollment();"><u>A</u>dd New Enrollment</button>&nbsp;
			<%
...
<!-- E N D : Main Container Table -->
    ...
    <%--  <html:hidden name="frmMember" property="portedYN"  value="<%=portedYN %>"/> --%>
    ...
    <input type="hidden" name="EnrollBatchSeqID" id="EnrollBatchSeqID" value="<%=EnrollBatchSeqID%>">
