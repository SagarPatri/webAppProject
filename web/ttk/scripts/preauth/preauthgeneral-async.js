   	...
   	document.getElementById('memberIdResult2').innerHTML='There is some problem while performing operations on the database.<br>Please report this problem to the administrator.';             
	     ...
	     // if(preauthMode=='EML1'||preauthMode==='EML1'||preauthMode=='FAX1'||preauthMode==='FAX1'||){
		  if(preauthMode ==='DHP'){       
		                        	...
		                        	var policyCategory = dataArray1[19];
		                    	...
		                    	document.getElementById('policyCategory').value=policyCategory;
	...
	$.ajax({
        ...
        success : function(data){
              ...
              if(rstatus==="EO"||rstatus.length==2){
              	...
              	var preCronYN=dataArray[4];
              	
              var proAuth=document.getElementById('productAuthority').value;
              	var primStatus=(primary==null||primary===""||primary==="YES")?false:true;
              	if(preCronYN=='Y'&&primStatus&&proAuth=='MOH'){
              		document.forms[1].chroPreValue.value="Y";
              		document.getElementById('preCronTypeID').style.display="";
              	}else {
              		document.getElementById('preCronTypeID').style.display="none";
              		document.forms[1].chroPreValue.value="N";
              	}
              ...
              //	if(diagSeqId==null||diagSeqId===""||diagSeqId.length<1){
              		document.getElementById('primaryAilment').checked=primStatus;
              	//}
              	}//if("SUCC")
        }
	});
...
function addObservs(activityDtlSeqId1){
	...
	if(authType==="PAT") 
	{
			pstatussi=document.forms[1].preAuthSeqID.value;
			modeType=document.forms[1].preAuthRecvTypeID.value;
	}	
	else if(authType==="CLM")
		
	{	
			pstatussi=document.forms[1].claimSeqID.value;
			modeType=document.forms[1].modeOfClaim.value;
	}
	var obrurl="/ObservationAction.do?obru="+ss+ss+ss+ss+"&mode=observWindow&activityDtlSeqId="+activityDtlSeqId1+"&pstatussi="+pstatussi+"&authType="+authType+"&modeType="+modeType;				
