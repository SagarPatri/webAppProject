function onUserSubmit()
{
  		...
  		if(!JS_SecondSubmit)
        {
  	    	...
  	    	if((document.forms[1].emplNumber.value=="")||(document.forms[1].emplNumber.value==null)){
  	    	
  	    	if((document.forms[1].sStartDate.value=="")||(document.forms[1].sStartDate.value==null)){
  	    	...
  	    	/*if((document.forms[1].emplNumber.value=="") ||(document.forms[1].emplNumber.value==null) ){
  	    		...
  	    		if(document.forms[1].sEndDate.value==""||(document.forms[1].sEndDate.value==null)){
...
function onOldLicense()
{
	document.forms[1].mode.value="onOldLicense";
	document.forms[1].reforward.value="oldLicenseList";
	document.forms[1].action="/EditHospitalSearchAction.do";
	document.forms[1].submit();
}
...
function isNumaricOnly(upObj){
	...
	if(objValue.length>=1){
		if(!re.test(objValue)){
			alert("Data should be in Numbers");
			upObj.value="";
			upObj.focus();
		}
	}  
...
function irdrgFactor()
{
	if(!JS_SecondSubmit)
    {
		...
		document.forms[1].mode.value="doIrdrgFactor";
    	document.forms[1].action="/EditHaadFactorsHospitalAction.do?hosp_seq_id="+hosp_seq_id;
    	document.forms[1].submit();
...
function changeNetworkTypeYN()
{
		...
		document.forms[1].action="/EditHaadFactorsHospitalAction.do?networkTypeYN="+networkTypeYN+"&networkType="+networkType;
		document.forms[1].submit();
}
...
function onAdd()
{	
	var frVal = document.getElementById("negotiationFactor").value;
	if(parseFloat(frVal) <1 || parseFloat(frVal)>3.25)
	{
			alert("Please enter Negotiation Factor between 1 to 3.25");
			document.getElementById("negotiationFactor").value="";
			document.getElementById("negotiationFactor").focus();
			return;
	}
	    ...
	    if( !((document.getElementById("startDate").value)==="") && !((document.getElementById("endDate").value)===""))
	   	{
	        ...
	        if(EndDate < Startdate)
	       	 {
	       	 	alert("Effective from date should be less than or equal to expiry date.");
		...
		var updateRemarks= document.getElementById("updateRemarks").value;
	    var space = updateRemarks.charCodeAt(0);
		  if(space==32)
		  {
				 alert("Update Remarks should not start with space.");
				 document.getElementById("updateRemarks").value="";
				 document.getElementById("updateRemarks").focus();
				 return;
		  }	
		  
		 if(updateRemarks.length != 0 && updateRemarks.length < 10 )
		 {	 
			 
				 alert("Update Remarks should have minimum of 10 characters.");
				 document.getElementById("updateRemarks").value="";
				 document.getElementById("updateRemarks").focus();
				 return;
		 }
	...
	if(!JS_SecondSubmit){
		document.forms[1].mode.value="AddIrdrgParameters";
    	document.forms[1].action = "/AddIrdrgParameters.do";
    	JS_SecondSubmit=true;
    	document.forms[1].submit();
   }
}
...
function onDelete(DhaNagSeqId){
	var hospSeqId= document.getElementById("hospSeqId").value;
	document.forms[1].mode.value="doDelete";
    document.forms[1].action = "/IrdrgParameters.do?DhaNagSeqId="+DhaNagSeqId+"&hospSeqId="+hospSeqId;
    document.forms[1].submit();

}


function onAuditLog(DhaNagSeqId)
{
		document.forms[1].mode.value="viewAuditLog";
    	document.forms[1].action="/IrdrgParameters.do?DhaNagSeqId="+DhaNagSeqId;
    	document.forms[1].submit();
}


function onCloseAuditLog()
{
	document.forms[1].mode.value="doCloseAuditLog";
    document.forms[1].action = "/IrdrgParameters.do";
    document.forms[1].submit();
}

 
function onEditIrdrg(dhaNagSeqId)
{
	
		document.forms[1].mode.value="editIrdrg";
    	document.forms[1].action="/IrdrgParameters.do?dhaNagSeqId="+dhaNagSeqId;
    	document.forms[1].submit();
}
...
function onClose()
{
	document.forms[1].mode.value="doCloseIrdrg";
