...
function onChangeClaimTypeGyncolgy()	{
	document.forms[0].mode.value="doChangeClaimTypeGyncolgy";
	document.forms[0].action="/DynGyncolgyRuleAction.do";	
	document.forms[0].submit();
}

function  selectEncounterTypes(aStyleID,aOPMode){ //GYC
	 var w = 300;
	 var h = 490;
var vConfigValues=document.getElementById(aStyleID).value;
	
	var path="/DynGyncolgyRuleAction.do?mode=openConfigWindow";
	var params="&windoType=Enc&spType=GYC&boxType=CH&windowTitle=Encounter List&configValues="+vConfigValues+"&displayID="+aStyleID+"&OPMode="+aOPMode;
...
function addGyncolgyDtls(){
	...
	if(vCountryID.indexOf("175")!=-1){
		...
		document.forms[0].mode.value="addGyncolgyDtls";
		document.forms[0].action="/DynGyncolgyAddRuleAction.do";
		document.getElementById("addGyncolgyDtlsBtnID").innerHTML="Please Wait Adding...";
		document.getElementById("addGyncolgyDtlsBtnID").disabled=true;
		document.forms[0].submit();
}

function editGyncolgyConf(argRowNum){
	if(!confirm("You want edit this configuration?")){
		return;
	}
	    document.forms[0].rowNum.value=argRowNum;
		document.forms[0].mode.value="editGyncolgyConf";
		document.forms[0].action="/DynGyncolgyRuleAction.do";		
		document.forms[0].submit();
}

function viewGyncolgyStaticConfDtls(aRowNum,aIdentity){	
	var w = 600;
	 var h = 200;
	
	var path="/DynGyncolgyRuleAction.do?mode=viewGyncolgyStaticConfDtls";
...
function deleteGyncolgyDtls(argRowNum){
	if(!confirm("You want delete this configuration?")){
		return;
	}
	    document.forms[0].rowNum.value=argRowNum;
		document.forms[0].mode.value="deleteGyncolgyDtls";
		document.forms[0].action="/DynGyncolgyRuleAction.do";		
		document.forms[0].submit();
}