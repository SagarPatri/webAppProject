...
function onChangeClaimTypeMinrSrgry()	{
	document.forms[0].mode.value="doChangeClaimTypeMinrSrgry";
	document.forms[0].action="/DynMinrSrgryRuleAction.do";	
	document.forms[0].submit();
}

function  selectEncounterTypes(aStyleID,aOPMode){ // MIS
	 var w = 300;
	 var h = 490;
var vConfigValues=document.getElementById(aStyleID).value;
	
	var path="/DynMinrSrgryRuleAction.do?mode=openConfigWindow";
	var params="&windoType=Enc&spType=MIS&boxType=CH&windowTitle=Encounter List&configValues="+vConfigValues+"&displayID="+aStyleID+"&OPMode="+aOPMode;
...
function addMinrSrgryDtls(){
	...
	if(vCountryID.indexOf("175")!=-1){
		...
		document.forms[0].mode.value="addMinrSrgryDtls";
		document.forms[0].action="/DynMinrSrgryAddRuleAction.do";
		document.getElementById("addMinrSrgryDtlsBtnID").innerHTML="Please Wait Adding...";
		document.getElementById("addMinrSrgryDtlsBtnID").disabled=true;
		document.forms[0].submit();
}

function editMinrSrgryConf(argRowNum){
	if(!confirm("You want edit this configuration?")){
		return;
	}
	    document.forms[0].rowNum.value=argRowNum;
		document.forms[0].mode.value="editMinrSrgryConf";
		document.forms[0].action="/DynMinrSrgryRuleAction.do";		
		document.forms[0].submit();
}

function viewMinrSrgryStaticConfDtls(aRowNum,aIdentity){	
	var w = 600;
	 var h = 200;
	
	var path="/DynMinrSrgryRuleAction.do?mode=viewMinrSrgryStaticConfDtls";
...
function deleteMinrSrgryDtls(argRowNum){
	if(!confirm("You want delete this configuration?")){
		return;
	}
	    document.forms[0].rowNum.value=argRowNum;
		document.forms[0].mode.value="deleteMinrSrgryDtls";
		document.forms[0].action="/DynMinrSrgryRuleAction.do";		
		document.forms[0].submit();
}