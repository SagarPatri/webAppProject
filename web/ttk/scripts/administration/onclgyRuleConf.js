...
function onChangeClaimTypeOnclgy()	{
	document.forms[0].mode.value="doChangeClaimTypeOnclgy";
	document.forms[0].action="/DynOnclgyRuleAction.do";	
	document.forms[0].submit();
}

function  selectEncounterTypes(aStyleID,aOPMode){ // ONC
	 var w = 300;
	 var h = 490;
var vConfigValues=document.getElementById(aStyleID).value;
	
	var path="/DynOnclgyRuleAction.do?mode=openConfigWindow";
	var params="&windoType=Enc&spType=ONC&boxType=CH&windowTitle=Encounter List&configValues="+vConfigValues+"&displayID="+aStyleID+"&OPMode="+aOPMode;
...
function addOnclgyDtls(){
	...
	if(vCountryID.indexOf("175")!=-1){
		...
		document.forms[0].mode.value="addOnclgyDtls";
		document.forms[0].action="/DynOnclgyAddRuleAction.do";
		document.getElementById("addOnclgyDtlsBtnID").innerHTML="Please Wait Adding...";
		document.getElementById("addOnclgyDtlsBtnID").disabled=true;
		document.forms[0].submit();
}

function editOnclgyConf(argRowNum){
	if(!confirm("You want edit this configuration?")){
		return;
	}
	    document.forms[0].rowNum.value=argRowNum;
		document.forms[0].mode.value="editOnclgyConf";
		document.forms[0].action="/DynOnclgyRuleAction.do";		
		document.forms[0].submit();
}

function viewOnclgyStaticConfDtls(aRowNum,aIdentity){	
	var w = 600;
	 var h = 200;
	
	var path="/DynOnclgyRuleAction.do?mode=viewOnclgyStaticConfDtls";
...
function deleteOnclgyDtls(argRowNum){
	if(!confirm("You want delete this configuration?")){
		return;
	}
	    document.forms[0].rowNum.value=argRowNum;
		document.forms[0].mode.value="deleteOnclgyDtls";
		document.forms[0].action="/DynOnclgyRuleAction.do";		
		document.forms[0].submit();
}