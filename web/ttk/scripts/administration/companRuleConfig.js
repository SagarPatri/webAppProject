...
function openGeoLocation(aStyleID,aOPMode){
	...
	var path="/DynCompanRuleAction.do?mode=openConfigWindow";
...
function addCompanDtls(){
	...
	if(vCountryID.indexOf("175")!=-1){
		...
		document.forms[0].mode.value="addCompanDtls";
		document.forms[0].action="/DynCompanAddRuleAction.do";
...
function deleteCompanDtls(argRowNum){
	if(!confirm("You want delete this configuration?")){
		return;
	}
	    document.forms[0].rowNum.value=argRowNum;
		document.forms[0].mode.value="deleteCompanDtls";
		document.forms[0].action="/DynCompanRuleAction.do";		
		document.forms[0].submit();
}
function editCompanConf(argRowNum){
	if(!confirm("You want edit this configuration?")){
		return;
	}
	    document.forms[0].rowNum.value=argRowNum;
		document.forms[0].mode.value="editCompanConf";
		document.forms[0].action="/DynCompanRuleAction.do";		
		document.forms[0].submit();
}
function viewCompanStaticConfDtls(aRowNum,aIdentity){	
		
		var w = 600;
		 var h = 200;
		
		var path="/DynCompanRuleAction.do?mode=viewCompanStaticConfDtls";
