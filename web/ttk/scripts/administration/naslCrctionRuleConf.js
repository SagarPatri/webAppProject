...
function onChangeClaimTypeNaslCrction()	{
	document.forms[0].mode.value="doChangeClaimTypeNaslCrction";
	document.forms[0].action="/DynNaslCrctionRuleAction.do";	
...
var vConfigValues=document.getElementById(aStyleID).value;
	...
	var path="/DynNaslCrctionRuleAction.do?mode=openConfigWindow";
	var params="&windoType=Enc&spType=NAS&boxType=CH&windowTitle=Encounter List&configValues="+vConfigValues+"&displayID="+aStyleID+"&OPMode="+aOPMode;
...
function addNaslCrctionDtls(){
	...
	if(vCountryID.indexOf("175")!=-1){
		...
		document.forms[0].mode.value="addNaslCrctionDtls";
		document.forms[0].action="/DynNaslCrctionAddRuleAction.do";
		document.getElementById("addNaslCrctionDtlsBtnID").innerHTML="Please Wait Adding...";
		document.getElementById("addNaslCrctionDtlsBtnID").disabled=true;
		document.forms[0].submit();
}

function editNaslCrctionConf(argRowNum){
	if(!confirm("You want edit this configuration?")){
		return;
	}
	    document.forms[0].rowNum.value=argRowNum;
		document.forms[0].mode.value="editNaslCrctionConf";
		document.forms[0].action="/DynNaslCrctionRuleAction.do";		
		document.forms[0].submit();
}

function viewNaslCrctionStaticConfDtls(aRowNum,aIdentity){	
	var w = 600;
	 var h = 200;
	
	var path="/DynNaslCrctionRuleAction.do?mode=viewNaslCrctionStaticConfDtls";
...
function deleteNaslCrctionDtls(argRowNum){
	if(!confirm("You want delete this configuration?")){
		return;
	}
	    document.forms[0].rowNum.value=argRowNum;
		document.forms[0].mode.value="deleteNaslCrctionDtls";
		document.forms[0].action="/DynNaslCrctionRuleAction.do";		
		document.forms[0].submit();
}