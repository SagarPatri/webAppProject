...
function onChangeClaimTypeOgnTrnspltRcpt()	{
	document.forms[0].mode.value="doChangeClaimTypeOgnTrnspltRcpt";
	document.forms[0].action="/DynOgnTrnspltRcptRuleAction.do";	
	document.forms[0].submit();
}

function  selectEncounterTypes(aStyleID,aOPMode){ // OTR
	 var w = 300;
	 var h = 490;
var vConfigValues=document.getElementById(aStyleID).value;
	
	var path="/DynOgnTrnspltRcptRuleAction.do?mode=openConfigWindow";
	var params="&windoType=Enc&spType=OTR&boxType=CH&windowTitle=Encounter List&configValues="+vConfigValues+"&displayID="+aStyleID+"&OPMode="+aOPMode;
...
function addOgnTrnspltRcptDtls(){
	var vCountryID=document.getElementById("countryID").value;
	if(vCountryID.indexOf("175")!=-1){
		...
		document.forms[0].mode.value="addOgnTrnspltRcptDtls";
		document.forms[0].action="/DynOgnTrnspltRcptAddRuleAction.do";
		document.getElementById("addOgnTrnspltRcptDtlsBtnID").innerHTML="Please Wait Adding...";
		document.getElementById("addOgnTrnspltRcptDtlsBtnID").disabled=true;
		document.forms[0].submit();
}

function editOgnTrnspltRcptConf(argRowNum){
	if(!confirm("You want edit this configuration?")){
		return;
	}
	    document.forms[0].rowNum.value=argRowNum;
		document.forms[0].mode.value="editOgnTrnspltRcptConf";
		document.forms[0].action="/DynOgnTrnspltRcptRuleAction.do";		
		document.forms[0].submit();
}

function viewOgnTrnspltRcptStaticConfDtls(aRowNum,aIdentity){	
	var w = 600;
	 var h = 200;
	
	var path="/DynOgnTrnspltRcptRuleAction.do?mode=viewOgnTrnspltRcptStaticConfDtls";
...
function deleteOgnTrnspltRcptDtls(argRowNum){
	if(!confirm("You want delete this configuration?")){
		return;
	}
	    document.forms[0].rowNum.value=argRowNum;
		document.forms[0].mode.value="deleteOgnTrnspltRcptDtls";
		document.forms[0].action="/DynOgnTrnspltRcptRuleAction.do";		
		document.forms[0].submit();
}